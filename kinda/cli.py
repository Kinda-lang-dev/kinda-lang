...
def validate_knda_file(file_path: Path) -> bool:
    ...
    try:
        ...
        safe_print(f"😵 Cannot access '{file_path}': {e}")
        safe_print("[tip] Check the file path and permissions")
        return False
    except Exception as e:
        safe_print(f"🤯 Unexpected error validating '{file_path}': {type(e).__name__}: {e}")
        return False
